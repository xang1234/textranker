...
Concrete, copy-paste examples help adoption more than theory. These recipes are meant to be
"grab-and-go" starting points for common real-world tasks.

Notes:
  * Outputs will vary by input text (and by tokenization if you use spaCy vs the built-in tokenizer).
  * If you want to tune phrase shapes (2-4 word phrases, noun-only phrases, grouping behavior, etc.),
    see the `TextRankConfig` section below.

### SEO / blog keywording (BaseTextRank + phrase grouping)
...
This recipe emphasizes:
  ...
  * noun/adjective/proper-noun phrases (to avoid noisy verbs),
  * scrubbed-text grouping (phrases look like what you'd actually put in a CMS / brief),
  * optional lemma-deduping (avoid near-duplicates in your final list).
...
### Academic abstract extraction (PositionRank)

Goal: extract keyphrases from paper abstracts (or news articles) where important terms are typically
introduced early. PositionRank biases the random walk toward early-occurring candidates, which often
matches academic writing style.

Tip: include the paper title + abstract together in one string (titles are short but highly informative).
...
### Security / privacy-focused extraction (BiasedTextRank)

Goal: pull out security- or privacy-relevant phrases from a mixed document (policies, architecture docs,
incident reports, DPIAs, vendor questionnaires). BiasedTextRank lets you "steer" extraction toward
your focus terms while still discovering related phrases.
...
### Topic-model guided keywords from LDA (TopicalPageRank + topic_weights_from_lda)

Goal: if you already run LDA (or any topic model) on a corpus, use the topic distribution to bias
keyword extraction automatically. TopicalPageRank uses a personalization vector where words with
higher topic weights get more teleport probability.
...
```
  ...
  * `topic_weights_from_lda` supports `aggregation` ("max" or "mean") and `top_n_words` to tune
    how per-topic weights get combined.

### Multi-topic documents (TopicRank / MultipartiteRank)

Goal: extract diverse phrases from long documents that cover multiple themes (e.g., quarterly reports,
RFCs, incident postmortems). Vanilla TextRank can over-focus on the dominant cluster.

Two strong options:
  * TopicRank: clusters candidates into topics, ranks topics, then selects representatives (diversity-first).
  * MultipartiteRank: keeps candidates but removes intra-topic edges + can boost early variants (more nuanced).
...
If you want even more diversity:
  * Split the document by headings/sections and run extraction per section (then merge/dedupe).
